...
import { getBuyerPaymentSettings, getCheckoutQuote } from "@/lib/api-client";
...
export default async function CheckoutPage({ searchParams }: CheckoutPageProps) {
  ...
  try {
    paymentSettings = (await getBuyerPaymentSettings(guestToken)).payload;
  } catch {
    paymentSettings = {
      stripe_enabled: true,
      cod_enabled: true,
  ...
  const anyPaymentEnabled = stripeEnabled || codEnabled;
      ...
      <div className="space-y-6">
        <header className="space-y-2">
          ...
          <h1 className="font-display text-4xl font-semibold leading-tight">
            Checkout is ready when your cart has items
          </h1>
          <p className="text-sm text-muted">
            Add items from one or more vendors, then return here.
          </p>
        </header>
  ...
  return (
    <div className="space-y-8">
      <header className="space-y-2">
        <p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted">Checkout</p>
        <h1 className="font-display text-4xl font-semibold leading-tight">
          Confirm multi-vendor order
        </h1>
        <p className="text-sm text-muted">
          One order, {quote.shipment_count} vendor shipments, independent fulfillment timelines.
        </p>
      </header>
      ...
      <section className="space-y-3">
        {quote.shipments.map((shipment) => (
          <article
            ...
            key={shipment.vendor_id}
          >
            <div className="flex items-center justify-between">
              <h2 className="font-display text-xl font-semibold">
                Vendor shipment {shipment.vendor_id}
              </h2>
      ...
      <aside className="rounded-md border border-border bg-white p-5">
        ...
        {stripeEnabled ? (
          ...
          <p className="mt-4 rounded-sm border border-border bg-surface px-3 py-2 text-xs text-muted">
        ...
        {codEnabled ? (
          ...
          <p className="mt-2 rounded-sm border border-border bg-surface px-3 py-2 text-xs text-muted">
        ...
        {anyPaymentEnabled ? (
          <p className="mt-2 text-xs text-muted">
            Stripe flow creates a payment intent and awaits secure confirmation; COD confirms order
            for payment on delivery.
          </p>
        ) : (
          <p className="mt-2 text-xs text-muted">
            No payment methods are enabled right now. Please contact support before placing this
            order.
          </p>
        )}
